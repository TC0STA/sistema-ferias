(() => {
  ...
  document.querySelectorAll('[data-confirm-action]').forEach(button => button.addEventListener('click', () => {
    ...
    document.getElementById('confirmTitle').textContent = cancel ? 'Cancelar solicitação?' : 'Confirmar desligamento?';
    document.getElementById('confirmMessage').textContent = cancel ? 'A solicitação permanecerá no histórico como CANCELADO.' : `Confirmar que o desligamento de ${button.dataset.confirmName} foi realizado?`;
    const submit = document.getElementById('confirmSubmit');
    submit.textContent = cancel ? 'Confirmar cancelamento' : 'Confirmar desligamento';
    submit.classList.toggle('danger', cancel);
    open(confirmModal);
  }));
})();
